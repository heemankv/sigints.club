...
- Stream detail page added with subscription flow (pubkey input).
- UI key generation, publish signal, and decrypt flow added for stream page.
...
- Deployed all three programs to devnet and created stream PDAs.
- SOLANA_STREAM_MAP populated in .env for stream-eth / stream-amazon / stream-anime.
...
- Stream registry enforced on-chain for subscribe + record_signal (active-only).
- Subscribe now creates mint + ATA via CPI in-program (avoids stack overflow).
- E2E updated to create stream registry PDAs before subscribing.
...
- Latest-only signal account PDA implemented (one account per stream).
