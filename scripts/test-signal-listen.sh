...
log "Starting backend (run-app.sh)..."
...
if [ ! -d "$ROOT/sdk/node_modules" ]; then
  log "Installing SDK dependencies..."
  npm -C "$ROOT/sdk" install
fi

log "Running SDK test scenario..."
export NODE_PATH="$ROOT/sdk/node_modules"
"$ROOT/sdk/node_modules/.bin/tsx" "$ROOT/scripts/test-signal-listen.ts"
